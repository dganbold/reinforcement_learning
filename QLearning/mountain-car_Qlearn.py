...
class QLearn:
    def __init__(self, actions, epsilon=0.1, alpha=0.2, gamma=0.9):
        ...
        self.Q = np.zeros((self.N_velocity,self.N_position*len(self.actions)))
        for s_2 in range(len(self.velocities)):
            for s_1 in range(len(self.positions)):
                self.Q[s_2, 3 * s_1 + 0] = -float('inf')
                self.Q[s_2, 3 * s_1 + 1] = -float('inf')
                self.Q[s_2, 3 * s_1 + 2] = -float('inf')
        #
    ...
    def chooseAction(self, state):
        ...
        return action
    #
    def chooseActionWithoutNoise(self, state):
        q = [self.getQ(state, a) for a in self.actions]
        maxQ = max(q)
        i = q.index(maxQ)
        action = self.actions[i]
...
if __name__ == "__main__":
    ...
    start_epsilon = 0.3   # exploration
    end_epsilon = 0.2     # exploration
    ...
    test_epoch = 10
    max_steps = 1000
    epsilons = np.linspace(start_epsilon, end_epsilon, train_epoch, endpoint=True)
    # ----------------------------------------
    # Actions
    # Type: Discrete(3)
    # Num | Observation
    ...
    for e in range(test_epoch):
        ...
        while (not game_over):
            ...
            # Greedy policy
            action = AI.chooseActionWithoutNoise(observation)
        ...
        if observation[0] > 0.5:
            ...
            print("#TEST Episode:{} finished after {} timesteps.".format(e, step))
        #
        # Plot
        plt.pause(1.5)
        AI.clearTrajectory()
